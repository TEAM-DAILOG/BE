    ...
    return applyDecorators(
        ...
        ApiBody({
            schema: {
                type: 'object',
                properties: {
                    isPush: { type: 'boolean', description: 'PUSH 알람 여부', example: true },
                    isDiary: { type: 'boolean', description: '일기 작성 알람 여부', example: true },
                    isDiaryReply: { type: 'boolean', description: '일기 답장 알람 여부', example: false },
                },
            },
        }),
        ApiResponse({
...
export function FindOneReminderSwagger() {
    return applyDecorators(
        ...
        ApiResponse({ status: 401, description: '사용자 인증 실패'}),
        ApiResponse({ status: 404, description: '리마인드 알림 설정을 찾을 수 없음'}),
        ApiResponse({ status: 500, description: '서버 내부 오류' }),
    );
}
    ...
    return applyDecorators(
        ...
        ApiBody({
            schema: {
                ...
                properties: {
                    fcmToken: { type: 'string', description: 'FCM 디바이스 토큰', example: 'eKxy1234...' },
                    deviceType: { type: 'string', description: '디바이스 종류', enum: ['IOS', 'ANDROID'], example: 'IOS' },
                },
            },
        }),
        ApiResponse({
